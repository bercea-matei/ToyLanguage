...
public class LatchTable<K,V> implements MyDict<K,V> {
    private Map<K,V> myLatch;
    private String dataTypeName = "LatchTable";

    public LatchTable() {
        this.myLatch = new HashMap<>();
    }
    @Override
    public synchronized V lookup(K id) throws IdNotFoundException {
        if (this.myLatch.containsKey(id)) {
            return this.myLatch.get(id);
        } else {
    ...
    public synchronized void add(K id, V val) throws IdAlreadyExistsException {
        if (! this.myLatch.containsKey(id)) {
            this.myLatch.putIfAbsent(id, val);
        } else {
    ...
    public synchronized MyDict<K,V> deepCopy() {
        MyDict<K,V> copy = new LatchTable<>();
        for (Map.Entry<K, V> entry_ : this.myLatch.entrySet())
        {
            try {
    ...
    public synchronized Iterator<Map.Entry<K, V>> iterator() {
        return new HashMap<>(this.myLatch).entrySet().iterator();
        /*
         * return new Iterator<Map.Entry<K, V>>() {
            private final Iterator<Map.Entry<K, V>> actualIterator = myLatch.entrySet().iterator();
            @Override
    ...
    public synchronized Map<K, V> getContent() {
        ...
        return new HashMap<>(this.myLatch); 
    }
    @Override
    public synchronized boolean isEmpty() {
        return this.myLatch.isEmpty();
    }
}
